...
* 28/02/26 [TASK] feat_deprecated_directive_properties — framework-level deprecation mechanism for directive types and option names. Plugins declare `deprecated_directive_types: ClassVar[list[str]]` and `deprecated_option_names: ClassVar[dict[str, list[str]]]`; `PluginBase.remap_deprecated_options()` mutates directives in-place. `PluginRegistry` gains `_by_deprecated_type` secondary index and `resolve_directive_type()`. `planner.create_plan()` calls `_remap_deprecated()` before validation so all downstream code sees canonical names. Concrete renames: `query-path` → `query_path` (directive type), `link` → `show_link`, `line_numbers_range` → `show_line_range` (file plugin options); old names produce WARNING, not error.

* 27/02/26 [TASK] tech_tutorial_plugin_improvements — three tutorial fixes from plugin-validator skill feedback. (1) Added `[build-system]` section (`requires = ["setuptools"]`, `build-backend = "setuptools.build_meta"`) to the `pyproject.toml` example — absence caused opaque `BackendUnavailable` error on `pip install -e .`. (2) Replaced one-liner registration intro with a bullet breakdown: entry points = discovery, `plugin_sequence` = load order; added `embedm -p` as the verification command. (3) Added environment-alignment callout before `pip install -e .` explaining the silent failure when system `pip` and project `.venv` embedm diverge.

* 27/02/26 [TASK] refactor_orchestration — lean orchestration.py by relocating domain/utility concerns to purpose-specific modules. `_compile_plan`, `_compile_plan_node`, `_build_directive_sequence`, `_output_file_compiled_dir`, `_dir_mode_compiled_dir`, `_emit_errors` moved to new `application/compiler.py`. `_glob_base`, `_extract_base_dir`, `_expand_directory_input`, `apply_line_endings` moved to `infrastructure/file_util.py` (functions made public; `expand_directory_input` gains `pattern="*.md"` default). `_build_context(config)` split into `_load_plugins(config) → (PluginRegistry, errors)` and `_build_context(config, registry) → EmbedmContext` for explicit plugin-load staging. `_output_target` renamed to `_format_output_label`. Three hardcoded strings in `_validate_plugin_config_schemas` replaced with `app_resources` entries. Test imports updated in `directory_mode_test.py`, `test_regression.py`, and `pipeline_test.py`.

* 27/02/26 [TASK] feat_plugin_debugging — `-p`/`--plugin-list` CLI flag + `PluginDiagnostics` service + WARNING on unresolved `plugin_sequence` entries. New `PluginDiagnostics.check()` returns `WARNING` statuses for `plugin_sequence` modules with no matching entry point. `_register_entry` extracted from `load_plugins` to keep complexity at rank B (7) after adding duplicate `directive_type` detection (ERROR). `_handle_plugin_list` in orchestration loads all enabled plugins, runs diagnostics, prints formatted report, exits 0/1. Startup warning path added alongside normal compilation flow. `present_warnings()` and `present_plugin_list()` added to `console.py`; `plugin_list: bool` field added to `Configuration` + `merge()`. 14 new tests across `cli_test.py`, `plugin_registry_test.py`, `plugin_diagnostics_test.py`, `console_test.py`.
...
* 25/02/26 [ARCH] tech_unify_directive_planning — the current fix for source-less directives (`_validate_sourceless_directives`) is a parallel code path rather than a unified one. The planner now has two separate functions for directives (with source → `_build_child`, without source → `_validate_sourceless_directives`) that both need to call `validate_input` and produce child nodes. The structural fix is a single `_plan_directive` function that handles both cases: call `validate_input`, on error return error node, if source recurse via `create_plan`, if source-less return leaf node; merge `_validate_sourceless_directives` and `_build_child`/`_validate_and_plan` into this single pathway. Backlog item created: `tech_unify_directive_planning`.

* 25/02/26 [TASK] fix source-less directives skipping validate_input — directives without a source (e.g. mermaid with inline options) never reached `_build_child`, so `validate_input` was never called and `artifact` was always None, crashing transform. Fix: added `_validate_sourceless_directives` in planner.py (called from `create_plan`) to call `validate_input("", ...)` for source-less directives and create child PlanNodes carrying the artifact. Updated `_find_or_create_node` in file_transformer.py to always consult `child_lookup` regardless of whether a source is present. Updated `test_create_plan_directive_without_source` to reflect the new correct behaviour (source-less directives with a registered plugin now produce a child node).
