"""Unit tests for apply_line_endings and verify_file_output."""
...
from embedm.application.verification import VerifyStatus, apply_line_endings, verify_file_output
...
def test_apply_line_endings_unknown_value_is_passthrough() -> None:
    ...
    text = "line\n"
    assert apply_line_endings(text, "lf") == text
...
def test_verify_file_output_missing_file(tmp_path: Path) -> None:
    from embedm.application.verification import VerifyStatus

    output_path = str(tmp_path / "out.md")
    status = verify_file_output("content\n", output_path, _config())
    assert status == VerifyStatus.MISSING
...
def test_verify_file_output_up_to_date(tmp_path: Path) -> None:
    ...
    assert status == VerifyStatus.UP_TO_DATE
...
def test_verify_file_output_stale(tmp_path: Path) -> None:
    ...
    status = verify_file_output("new content\n", str(output_path), _config())
    assert status == VerifyStatus.STALE
...
def test_verify_file_output_crlf_up_to_date(tmp_path: Path) -> None:
    ...
    status = verify_file_output("line one\nline two\n", str(output_path), _config("crlf"))
    assert status == VerifyStatus.UP_TO_DATE
