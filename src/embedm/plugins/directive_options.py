...
def validate_option(directive: Directive, name: str, cast: Callable[[str], Any]) -> Status | None:
    """Check if the option can be cast to the requested type. Returns Status on failure, None if valid or absent."""
    value = directive.options.get(name)
    return None if value is None else _validate_cast(name, value, cast)


def get_option(directive: Directive, name: str, cast: Callable[[str], T], default_value: T) -> T:
